...
#include "jemalloc/internal/shambles.h"
//#include <execinfo.h>
...
static int shambles_inited_n = 1;

static void nopReportAlloc(AllocEventType type, void *in, void *out, size_t size){}

void (*reportAllocFunc)(AllocEventType, void *, void *, size_t) = &nopReportAlloc;

static void shambles_init(){
	void *handle;
	void (*init)(void (**reportAllocFunc)(AllocEventType, void *, void *, size_t));
	const char *lib = getenv("SHAMBLES_PLUGIN");
	handle = dlopen(lib, RTLD_NOW);
	if(handle != NULL){
		init = dlsym(handle, "shambles_init");
		if(init != NULL){
			init(&reportAllocFunc);
		}
	}
}

void logAlloc(AllocEventType type, void *in, void *out, size_t size){
	if(malloc_init_state != malloc_init_initialized || __atomic_load_n(&shambles_inited_n, __ATOMIC_RELAXED)){
		if(__atomic_exchange_n(&shambles_inited_n, 2, __ATOMIC_RELAXED) == 1){
			shambles_init();
			__atomic_store_n(&shambles_inited_n, 0, __ATOMIC_RELAXED);
		}
	}else{
		reportAllocFunc(type, in, out, size);
	}
}
...
je_posix_memalign(void **memptr, size_t alignment, size_t size) {
	...
	LOG("core.posix_memalign.exit", "result: %d, alloc ptr: %p", ret,
	    *memptr);
	    fprintf(stderr, "%ld\n", size);
	logAlloc(ALLOC_EVENT_ALLOC, NULL, *memptr, size);
	return ret;
}
...
je_aligned_alloc(size_t alignment, size_t size) {
	...
	logAlloc(ALLOC_EVENT_ALLOC, NULL, ret, size);
...
je_realloc(void *ptr, size_t size) {
	...
	if (likely(ptr != NULL && size != 0)) {
		...
		logAlloc(ALLOC_EVENT_REALLOC, ptr, ret, size);
		return ret;
