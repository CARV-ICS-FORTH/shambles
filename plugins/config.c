...
int ShamblesPluginConfigInit(struct ShamblesPluginConfig *config){
	...
	if((env = getenv("SHMBLES_FAST_NODEMASK"))==NULL){
	...
	if((env = getenv("SHMBLES_SLOW_NODEMASK"))==NULL){
		config->slowmask[0] = 0;
	}else{
	...
	if((env = getenv("SHMBLES_CHUNK_SIZE"))==NULL){
		//not defined, try to get it form SHMBLES_FAST_MEM_SIZE/SHMBLES_FAST_MEM_CHUNKS
		if((env = getenv("SHMBLES_FAST_MEM_SIZE"))==NULL){
			...
			if(!config->chunkSize){
				...
				if((env = getenv("SHMBLES_FAST_MEM_CHUNKS"))==NULL){
					config->chunkSize = (size_t)(-1);
	...
	if((env = getenv("SHMBLES_FAST_MEM_CHUNKS"))==NULL){
		//not defined, try to get it form SHMBLES_FAST_MEM_SIZE/SHMBLES_CHUNK_SIZE
		if((env = getenv("SHMBLES_FAST_MEM_SIZE"))==NULL){
